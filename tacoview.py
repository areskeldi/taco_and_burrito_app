...
class TacoView(MapView):
    ...
    def get_tacos_in_fov(self, *args):
        ...
        sql_statement = "SELECT DISTINCT name, longitude, latitude FROM tacos_and_burritos WHERE longitude > %s AND longitude < %s AND latitude > %s AND latitude < %s "%(min_lon, max_lon, min_lat, max_lat)
